...
def render_table(header_row, data, delim=False, extra_gap=0, hide_empty=False):
    ...
    def get_max_lens(rows):
        cols = list(zip(*rows, strict=False)
                    ) if rows else [[] for _ in range(ncols)]
        return [max((width(v) for v in col), default=0) for col in cols]

    if hide_empty:
        max_lens = get_max_lens(data)
        keep = [ml > 0 for ml in max_lens]
        header_row = [c for k, c in zip(keep, header_row, strict=False) if k]
        data = [[c for k, c in zip(keep, r, strict=False) if k] for r in data]
        ncols = len(header_row)
    ...
    lines = [
        (' ' * extra_gap).join(header[:1] + list(
            itertools.chain.from_iterable([vbar, c] for c in header[1:]))),
    ]
    ...
    for r in body:
        lines.append(' ' * extra_gap + ' '.join(r[:1] + list(
            itertools.chain.from_iterable([vbar, c] for c in r[1:]))))
