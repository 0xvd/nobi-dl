...
class Resolve_FMTS(ExtractorBase):
    def __init__(self, md, info_dict, fmt: str | None = None, formats: dict | None = None):
        super().__init__(md)
    ...
    def _google_resolver(self, url, need_dict=False):
        ...
        google_head = self._request(
            url, method='HEAD', note='Ping Google server')
    ...
    def _parse_fsl(self, url: str, format_id: str | None = None):
        ...
        filename = filename or url_filename(
            real_url) or url_filename(url) or 'download'
    ...
    def _parse_direct_drive(self, url):
        ...
        webpage = self._download_webpage(
            url, note='Downloading hubdrive webpage')
        ...
        if not did:
            did = _search_regex(
                r'\bdown-id[^>]*>\s*([^<\s]+)', webpage, default=None)
    ...
    def drive_seed(self, url):
        ...
        webpage = self._download_webpage(
            url, note='Downloading drive seed webpage')
        if '/file' not in url:
            path = _search_regex(
                r'''['\"]+((?:https://)?/?fil[^'\"]+)''', webpage)
