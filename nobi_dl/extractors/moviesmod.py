...
class MoviesModME(ExtractorBase):
    _IE_NAME = 'Moviesmod'
    _VALID_URL = r'https://?(?:moviesmod|animeflix)\.[^/]+/[^?&#]+'
    _SEARCH = True
    ...
    def extract_h3_text(self, block):
        ...
        return re.sub(r'<[^>]+', ' ', h3_text)

    def redirector(self, url):
        if '?url' not in url:
            return url
        encoded_url = _search_regex(r'\?url=([^?&#]+)', url)
        if 'http' in encoded_url:
            return encoded_url
        return b64d(encoded_url)

    def extract_episodes(self, url, h3_text, op_webpage):
        url = self.redirector(url) if '?url' in url else url
        ...
        for atag in _parse_a_tags(webpage):
            ...
            if not any(k in label for k in ('episode', 'episo', 'sode')):
                continue
            if not any(
                k in href for k in ('modpro.blog', 'unblockedgames', 'episodes')
            ):
                continue
            if any(k in label for k in ('episodes batch', 'sodes batch')):
                continue
            if 'episodes' in href:
                continue
    ...
    def entries(self, block, h3tag, op_webpage):
        ...
        for atag in _parse_a_tags(block):
            ...
            label = (name or '').lower()
            if not any(
                k in label for k in ('episode', 'episode links', 'gdrive', 'drive')
            ):
                continue
