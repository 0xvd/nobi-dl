...
class Hdhub4uME(ExtractorBase):
    ...
    def _hdhub_url_decoder(self, url):
        ...
        if not data:
            return None
        return b64d(data['o']) if data else None
    ...
    def _parse_hubcloud(self, url):
        ...
        webpage = self._download_webpage(
            url, 'Downloading hubcloud information')
    ...
    def _parse_drive(self, url):
        ...
        self._GLOBAL_SEEN_URLS.add(url)
        webpage = self._download_webpage(
            url, 'Downloading hubdrive information')
        hubcloud_url = _search_regex(
            self._VALID_HUBCLOUD_URL, webpage, default=None)
    ...
    def _parse_archives(self, url):
        ...
        self._GLOBAL_SEEN_URLS.add(url)
        webpage = self._download_webpage(
            url, 'Downloading hub archieve information')
        hub_url = _search_regex(
            self._VALID_HUBCLOUD_URL, webpage, default=None)
        if hub_url:
    ...
    def _minochinos_js_decoder(self, webpage: str):
        ...
        p = ast.literal_eval(
            m.group('q1') + m.group('payload') + m.group('q1'))
        base = int(m.group('base'))
        k_raw = ast.literal_eval(
            m.group('q2') + m.group('dict') + m.group('q2'))
        k = k_raw.split('|')
    ...
    def _extract_player_formats(self, direct_url_or_webpage):
        ...
        video_webpage = self._download_webpage(
            video_url, 'Downloading player url')
        ...
        if not js_data:
            ...
            return []
        video_urls = _search_json(
            r'(?:urls|Urls)\s*=\s*(\{[\s\S]+?);', js_data)
    ...
    def _get_movie(self, url_or_webpage):
        ...
        title = _search_regex(
            r'\bheadline\s*"\s*:\s*"([^"]+)"', webpage, default='')
    ...
    def hublinks(self, url):
        formats = []
        webpage = self._download_webpage(
            url, note='Download hublinks information')
        for h3_or_h5 in re.findall(r'<(?:h3|h5)[\s\S]+?<\/(?:h3|h5>)', webpage):
            h3_or_h5_title = _search_regex(
                r'<(?:h3|h5)[^>]+>([^<]+)<', h3_or_h5)
            if h3_or_h5_title:
    ...
    def diff_entries(self, webpage):
        valid_url_keys = ('drive', 'hubdrive', 'hdstream',
                          'stream', '?id=', 'hubcloud')
        entries = []
    ...
    def entries(self, webpage):
        ...
        for block in blocks:
            ...
            season = _search_regex(
                r'(?is)(?:season|s0|s)\s*(\d+)', block, default=1)
            if not ep:
                continue
            ...
            for atag in _parse_a_tags(block):
                ...
                href = self._hdhub_url_decoder(
                    href) if '?id=' in href else href
