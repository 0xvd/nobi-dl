...
class FourkhdhubME(ExtractorBase):
    ...
    def _clean_meta(self, meta: str) -> str:
        ...
        return re.compile(r'\s*•\s*').sub(' • ', meta)
    ...
    def _parse_search_4k_atags(self, atag: str, host: str):
        ...
        year = _search_regex(
            r'\b(19\d{2}|20\d{2})\b', meta, default='').strip()
    ...
    def entries(self, webpage):
        ...
        for season in re.findall(
            ...
            format_id = _search_regex(
                r'\bfile-title[^>]+>([^<]+)<', season).strip()

            ep = _search_regex(
                r'(?is)(?:s\d+\s*)?(?:episode|ep|e)\s*(\d+)', format_id)
            season_num = _search_regex(
                r'(?is)(?:season|s0|s)\s*(\d+)', season) or '1'
